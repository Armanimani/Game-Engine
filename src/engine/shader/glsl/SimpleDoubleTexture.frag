...
uniform sampler2D matDiffuseTexture1;
uniform sampler2D matDiffuseTexture2;
...
vec4 calculateAmbient()
{
	...
	for (int i = 0; i < ambientLightSize; ++i)
	{
		...
		ambient += matKa * ambientLight[i].color * ambientLight[i].intensity * attenuationFactor;
	}
	return ambient;
}
...
vec4 calculateDiffuse()
{
	...
	for (int i = 0; i < pointLightSize; ++i)
	{
		...
		diffuse += matKd * pointLight[i].diffuseColor * pointLight[i].diffuseIntensity  * sDotN * attenuationFactor ;
	}
	...
	for (int i = 0; i < directionalLightSize; ++i)
	{
		...
		toLightNormal = normalize(toLightVector);
		sDotN = max(dot(normalize(surfaceNormal), toLightNormal), 0.0);
		diffuse += matKd * directionalLight[i].diffuseColor * directionalLight[i].diffuseIntensity  * sDotN;
	}
	...
	for (int i = 0; i < spotLightSize; ++i)
	{
		...
		if( angle < cutoff)
		{
			...
			diffuse += matKd * spotLight[i].diffuseColor * spotLight[i].diffuseIntensity  * sDotN * spotFactor * attenuationFactor;
		}
	}
...
void main()
{
	vec4 matColor1 = texture(matDiffuseTexture1, textureCoords);
	vec4 matColor2 = texture(matDiffuseTexture2, textureCoords);
	vec4 matColor = mix(matColor1, matColor2, matColor2.a);
	vec4 linearColor = matColor *(calculateAmbient() + calculateDiffuse()) + matSpecularColor * calculateSpecular();
