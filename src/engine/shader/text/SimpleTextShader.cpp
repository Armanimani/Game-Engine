#include "SimpleTextShader.h"

void SimpleTextShader::render(const std::shared_ptr<GUITextModel> model)
{
	start();
...
void SimpleTextShader::loadAllToUniform(const std::shared_ptr<GUITextModel> model)
{
	loadToUniform(location_color, model->getMaterial()->getProperties().color);
}

void SimpleTextShader::getAllUniformLocations()
{
	std::string temp;
